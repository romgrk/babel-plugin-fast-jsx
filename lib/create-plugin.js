  ...
  return (0, _helperPluginUtils.declare)((_, options) => {
    ...
    if (process.env.BABEL_8_BREAKING) {
      if ("useSpread" in options) {
        throw new Error('@babel/plugin-transform-fast-react-jsx: Since Babel 8, an inline object with spread elements is always used, and the "useSpread" option is no longer available. Please remove it from your config.');
      }
      if ("useBuiltIns" in options) {
        const useBuiltInsFormatted = JSON.stringify(options.useBuiltIns);
        throw new Error(`@babel/plugin-transform-fast-react-jsx: Since "useBuiltIns" is removed in Babel 8, you can remove it from the config.
- Babel 8 now transforms JSX spread to object spread. If you need to transpile object spread with
\`useBuiltIns: ${useBuiltInsFormatted}\`, you can use the following config
{
  "plugins": [
    "@babel/plugin-transform-fast-react-jsx"
    ["@babel/plugin-transform-object-rest-spread", { "loose": true, "useBuiltIns": ${useBuiltInsFormatted} }]
      ...
      if (filter != null && RUNTIME_DEFAULT === "automatic") {
        throw new Error('@babel/plugin-transform-fast-react-jsx: "filter" option can not be used with automatic runtime. If you are upgrading from Babel 7, please specify `runtime: "classic"`.');
      }
    } else {
      var {
        useSpread = false,
      ...
      if (RUNTIME_DEFAULT === "classic") {
        if (typeof useSpread !== "boolean") {
          throw new Error("transform-fast-react-jsx currently only accepts a boolean option for " + "useSpread (defaults to false)");
        }
        if (typeof useBuiltIns !== "boolean") {
          throw new Error("transform-fast-react-jsx currently only accepts a boolean option for " + "useBuiltIns (defaults to false)");
        }
        if (useSpread && useBuiltIns) {
          throw new Error("transform-fast-react-jsx currently only accepts useBuiltIns or useSpread " + "but not both");
        }
      }
    }
...
You can set \`throwIfNamespace: false\` to bypass this warning.`);
    ...
    function buildJSXElementCall(path, file) {
      ...
      args.push(attribs);
      if (development) {
        args.push(extracted.key ?? path.scope.buildUndefinedNode(), _core.types.booleanLiteral(children.length > 1));
